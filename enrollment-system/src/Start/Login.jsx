...
import CvImage from '../assets/images/Cvsu.jpg';
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (isRegistering) {
      ...
      const newUser = {
        email: emailOrUserID,
        password: password,
        role: 'Student',  // You can adjust this role if needed
      };
  
      // Simulating registration success (no API call)
      setPopupMessage('Registration successful!');
      setEmailOrUserID('');
      setPassword('');
      setRetypePassword('');
      ...
      // Hardcoded login data for validation
      const hardcodedUsers = [
        { email: 'student@gmail.com', password: 'studentpass1', role: 'Student' },
        { email: 'bc.student@cvsu.edu.ph', password: 'studentpass', role: 'Student' },
        { email: 'admin@cvsu.edu.ph', password: 'adminpass', role: 'Admin' },
        { email: 'officer@cvsu.edu.ph', password: 'officerpass', role: 'Officer' },
      ];
  
      // Find the user from the hardcoded data
      const user = hardcodedUsers.find(
        (u) => u.email === emailOrUserID && u.password === password
      );
  
      if (user) {
        setUser({ email: emailOrUserID, role: user.role });
        setPopupMessage('Login successful!');
  
        // Store the user data in localStorage
        localStorage.setItem('user', JSON.stringify({ email: emailOrUserID, role: user.role }));
